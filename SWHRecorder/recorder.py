...
class SwhRecorder:
    ...
    def __init__(self):
        ...
        self.newAudio=False
        pi = numpy.pi
        sinew = numpy.sin(2.*pi*440.*numpy.arange(0, .0929,1./44100))[:-1]
        M = 2
        self.X = toeplitz(sinew, numpy.zeros(M))
        self.R_inv = inv( numpy.dot( self.X.transpose(), self.X )  )
    ...
    def fft(self,data=None,trimBy=10,logScale=False,divBy=100):
        ...
        # filter the signal adaptively with lms
        self.W = numpy.dot(self.R_inv , numpy.dot(self.X.transpose(),data) )
        #y = numpy.dot(self.X, W)
        y = numpy.dot(self.X, self.W)
        data = data - y
        #self.W = self.W + .0001*numpy.dot(self.X.transpose(),data)
        #print data.shape
